...
func TestApplyUpdateNode(t *testing.T) {
	target := common.ID{}
	deltDeletes:=&roaring.Bitmap{}
	baseDelets:=&roaring.Bitmap{}
	baseDelets.Add(1)
	node:=NewUpdateNode(&target,nil,baseDelets,deltDeletes,nil)
	node.Update(0,[]byte("update"))
	node.DeleteLocked(3,4)
